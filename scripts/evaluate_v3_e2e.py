"""
Hour 2: Comprehensive Multi-Cohort & In-Domain Retention Evaluation for V3-E2E.
Evaluates `checkpoints/v3_e2e_multisource/best_model.pth` on:
...
Outputs: results/benchmark_eval_v3/v3_e2e_comprehensive_eval.json
"""
...
logger = logging.getLogger("v3_e2e_eval")
...
def main():
    device = get_device()
    logger.info(f"Starting Hour 2 Evaluation of V3-E2E Model on {device}")
    ...
    results = {
        "model": "DeepTrace (v3_e2e_multisource)",
    ...
    # 2. FaceForensics++ Manifest Cohorts
    ...
    real_paths = df_ffpp[df_ffpp["manipulation_type"] == "real"][col].tolist()[5000:7000]  # held-out 2,000 reals
    ...
    for manip in manip_types:
        fake_paths = df_ffpp[df_ffpp["manipulation_type"] == manip][col].tolist()[5000:7000]  # held-out 2,000 fakes
    ...
    # 3. Overall FF++ (N=14,000: 2,000 Real + 12,000 Fake)
    ...
    # Save to JSON
    out_file = Path("results/benchmark_eval_v3/v3_e2e_comprehensive_eval.json")
    out_file.parent.mkdir(parents=True, exist_ok=True)
    with open(out_file, "w") as f:
        json.dump(results, f, indent=2)

    logger.info(f"=== Comprehensive Evaluation Saved: {out_file} ===")
