"""
Post-Training Evaluation of DeepTrace (v3_multisource_finetune) on FaceForensics++ C23.
Evaluates across all 6 manipulation subsets and computes overall metrics.
"""
...
logger = logging.getLogger("ffpp_v3_eval")
...
def main():
    device = get_device()
    logger.info(f"Running Post-Fine-Tuning FaceForensics++ Benchmark on {device}")

    # Load DeepTrace V3 Multi-Source Checkpoint
    ckpt_path = "checkpoints/v3_multisource_finetune/best_model.pth"
    ...
    # Sample 2,000 real images for evaluation
    ...
    for m in manip_types:
        ...
        logger.info(f"Evaluating V3 Model: Real vs. {m} (N={len(subset_records)})...")
        res = evaluate_records(model, subset_records, device)
        results_per_method[m] = res
        logger.info(f"  --> {m} Post-FT Acc: {res['accuracy']*100:.2f}%, AUC: {res['roc_auc']:.4f}")
    ...
    logger.info(f"=== OVERALL FF++ POST-FT ACCURACY: {overall_res['accuracy']*100:.2f}%, ROC-AUC: {overall_res['roc_auc']:.4f} ===")

    final_report = {
        "model": "DeepTrace (v3_multisource_finetune)",
        "checkpoint": ckpt_path,
        "overall": overall_res,
        "per_manipulation": results_per_method,
    }

    out_file = Path("results/benchmark_eval_v2/ffpp_v3_multisource_eval.json")
    out_file.parent.mkdir(parents=True, exist_ok=True)
    with open(out_file, "w") as f:
        json.dump(final_report, f, indent=2)

    logger.info(f"FF++ V3 results saved to: {out_file}")
