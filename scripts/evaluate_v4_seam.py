"""
Evaluation script for V4-Seam Hard-Mined model.
Evaluates `checkpoints/v4_seam_hardmining/best_model.pth` on:
...
Outputs: results/benchmark_eval_v4/v4_seam_evaluation.json
"""
...
logger = logging.getLogger("v4_seam_eval")
...
def main():
    device = get_device()
    logger.info(f"Starting Evaluation of V4-Seam Model on {device}")

    ckpt_path = "checkpoints/v4_seam_hardmining/best_model.pth"
    ...
    results = {
        "model": "DeepTrace (v4_seam_hardmining)",
    ...
    # Save to JSON
    out_file = Path("results/benchmark_eval_v4/v4_seam_evaluation.json")
    out_file.parent.mkdir(parents=True, exist_ok=True)
    with open(out_file, "w") as f:
        json.dump(results, f, indent=2)

    logger.info(f"=== V4-Seam Evaluation Complete & Saved: {out_file} ===")
